...
plt.figure()
plt.plot(N_D, pv.si.resistivity_Si_n(N_D), label='n-type')
plt.plot(N_D, pv.si.resistivity_Si_p(N_D), label='p-type')
plt.loglog()
plt.legend()
...
plt.ylim(1e-4, 1e4)
# plt.show()
delta_n = pv.cell.implied_carrier(0.7, 1e15)
print(pv.cell.impliedV(1e15, 6.7e15))
G = pv.cell.current2gen(0.038)
tau = delta_n / G
print(delta_n, G, tau)

# plot the mobility models
N_D = np.logspace(14, 22)
mmobility = pv.si.mob_masetti_phos(N_D)
